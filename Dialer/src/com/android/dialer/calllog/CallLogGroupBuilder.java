...
public class CallLogGroupBuilder {
    ...
    public void addGroups(Cursor cursor) {
        ...
        int firstVideoFlag = cursor.getInt(CallLogQuery.VIDEO_CALL_FLAG);// SPRD:
        while (cursor.moveToNext()) {
            ...
            final int videoFlag = cursor.getInt(CallLogQuery.VIDEO_CALL_FLAG);// SPRD:
            final boolean sameNumber = equalNumbers(firstNumber, currentNumber);
            final boolean sameVideoFlag = (firstVideoFlag == videoFlag);// SPRD:
            final boolean shouldGroup;

            if (!sameNumber||!sameVideoFlag) {
                // Should only group with calls from the same number.
                shouldGroup = false;
            } else if (firstCallType == Calls.VOICEMAIL_TYPE) {
                // Voicemail should only be grouped singly.
                shouldGroup = callType == Calls.MISSED_TYPE;// SPRD:
            } else {
